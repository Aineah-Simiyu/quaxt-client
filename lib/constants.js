...
export const FILE_UPLOAD = {
  ...
  ALLOWED_TYPES: ['image/jpeg', 'image/png', 'application/pdf', 'application/zip', 'text/plain']
};
...
export const isInstructorOrAdmin = (user) => {
  return hasRole(user, [ROLES.TRAINER, ROLES.ADMIN, ROLES.SCHOOL_ADMIN]) || 
         hasRole(user, ['instructor', 'admin']); // Legacy support
};

// Helper function to check if user is admin
export const isAdmin = (user) => {
  return hasRole(user, [ROLES.ADMIN, ROLES.SCHOOL_ADMIN]) || 
         hasRole(user, 'admin'); // Legacy support
};

// Check if user is a school admin
export const isSchoolAdmin = (user) => {
  return user && (user.role === ROLES.SCHOOL_ADMIN || user.role === 'school_admin');
};
