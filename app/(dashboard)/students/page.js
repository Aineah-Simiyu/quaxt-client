...
function StudentsPage() {
    ...
    for (const cohort of allCohorts) {
      if (cohort.students && Array.isArray(cohort.students)) {
        for (const studentRef of cohort.students) {
          const studentId =
            typeof studentRef === "string" ? studentRef : studentRef._id;
          ...
          if (studentMap.has(studentId)) {
            ...
            const totalProgress = student.cohorts.reduce(
              (sum, c) => sum + c.progress,
              0,
            );
            student.overallProgress = Math.round(
              totalProgress / student.cohorts.length,
            );
          }
        }
      }
  ...
  const calculateAnalyticsStats = (analytics, studentsCount) => {
    ...
    try {
      ...
      setStats((prevStats) => ({
        ...
        completionRate:
          studentsCount > 0
            ? Math.round((prevStats.activeStudents / studentsCount) * 100)
            : 0,
        averageProgress: 0,
      }));
    }
  ...
  const handleManageStudent = (student) => {
    ...
    setAvailableCohorts(
      cohorts.filter(
        (cohort) => !student.cohorts?.some((sc) => sc._id === cohort._id),
      ),
    );
  ...
  const handleRemoveFromCohort = async (cohortId) => {
    ...
    try {
      await cohortService.removeStudentFromCohort(
        cohortId,
        selectedStudent._id,
      );
    ...
    try {
      ...
      if (cohort && cohort !== "none" && createdUser._id) {
        try {
          ...
          toast.success(
            `Student invitation sent to ${email.trim()} and added to cohort`,
          );
        } catch (cohortError) {
          console.error("Error adding student to cohort:", cohortError);
          toast.success(
            `Student invitation sent to ${email.trim()}, but failed to add to cohort`,
          );
        }
      } else {
        ...
        <Card className="border-slate-200 bg-white">
                  ...
                  >
                    ...
                    <DialogContent className="sm:max-w-md">
                      ...
                      <div className="space-y-4">
                        ...
                        <div>
                          <Label
                            htmlFor="createCohort"
                            className="text-sm font-medium text-slate-700"
                          >
                            Assign to Cohort (Optional)
                          </Label>
                          <Select
                            value={createStudentData.cohort}
                            onValueChange={(value) =>
                              setCreateStudentData((prev) => ({
                                ...prev,
                                cohort: value,
                  ...
                  >
                    ...
                    <DialogContent className="sm:max-w-md">
                      <DialogHeader>
                        ...
                        <DialogDescription className="text-slate-600">
                          Upload a CSV file to add multiple students to a cohort
                          at once
                        </DialogDescription>
                      </DialogHeader>
                      <div className="space-y-4">
          ...
          >
            <DialogContent className="sm:max-w-2xl max-h-[90vh] overflow-y-auto">
              <DialogHeader>
                <DialogTitle className="text-slate-900">
                  Manage Student: {selectedStudent?.firstName}{" "}
                  ...
                  <div className="space-y-4">
                    <div className="p-4 border border-red-200 rounded-lg bg-red-50">
                      ...
                      <p className="text-sm text-red-600 mb-3">
                        Deleting a student will permanently remove their account
              ...
              <Table>
                ...
                <TableBody>
                  {filteredStudents.length === 0 ? (
                    <TableRow>
                      <TableCell
                        colSpan={isSchoolAdmin(user) ? 7 : 6}
                    ...
                    filteredStudents.map((student) => {
                      ...
                      const joinedDate = new Date(
                        student.joinedDate || student.createdAt || Date.now(),
                      ).toLocaleDateString();
                      const fullName =
                        `${student.firstName || ""} ${student.lastName || ""}`.trim() ||
                        student.name ||
        ...
        <Dialog open={isViewDialogOpen} onOpenChange={setIsViewDialogOpen}>
          <DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto">
            <DialogHeader>
              <DialogTitle className="flex items-center space-x-3">
                <Avatar className="h-10 w-10">
                  ...
                  <AvatarFallback className="text-lg font-medium">
                    {viewStudent?.firstName?.charAt(0) || ""}
                    {viewStudent?.lastName?.charAt(0) || ""}
                  </AvatarFallback>
                </Avatar>
                <div>
              ...
              <DialogDescription>Student Details - View Only</DialogDescription>
            </DialogHeader>
                  ...
                  <div>
                    ...
                    <div className="flex items-center space-x-2 mt-1">
                      <div className="w-20 bg-slate-200 rounded-full h-2">
                        <div
                          className={`h-2 rounded-full ${
                            (viewStudent.overallProgress || 0) >= 80
                              ? "bg-emerald-500"
                              : (viewStudent.overallProgress || 0) >= 60
                                ? "bg-blue-500"
                                : (viewStudent.overallProgress || 0) >= 40
                                  ? "bg-amber-500"
                                  : "bg-slate-400"
                          }`}
                          style={{
                            width: `${Math.min(viewStudent.overallProgress || 0, 100)}%`,
                          }}
                        ></div>
                      </div>
                      <span className="text-sm text-slate-600">
                        {viewStudent.overallProgress || 0}%
                      </span>
                    </div>
                  </div>
                </div>
                              ...
                              <div className="w-16 bg-slate-200 rounded-full h-1.5">
                                <div
                                  className={`h-1.5 rounded-full ${
                                    (cohort.progress || 0) >= 80
                                      ? "bg-emerald-500"
                                      : (cohort.progress || 0) >= 60
                                        ? "bg-blue-500"
                                        : (cohort.progress || 0) >= 40
                                          ? "bg-amber-500"
                                          : "bg-slate-400"
                                  }`}
                                  style={{
                                    width: `${Math.min(cohort.progress || 0, 100)}%`,
                                  }}
                                ></div>
                              </div>
